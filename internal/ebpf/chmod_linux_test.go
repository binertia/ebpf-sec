...
func TestNormalizeChmodFchmodatRecord(t *testing.T) {
	procRoot := t.TempDir()
	...
	if got := event.Metadata["syscall"]; got != "fchmodat" {
		t.Fatalf("syscall = %#v, want fchmodat", got)
	}
...
func TestNormalizeChmodFchmodatRecordIncludesFailure(t *testing.T) {
	raw := chmodRecord{PID: 4112, FD: atFDCWD, Mode: 0o755, Syscall: fchmodatSyscallNumber, ReturnValue: -1}
	copy(raw.FilePath[:], "/tmp/payload")

	collector := &ChmodCollector{host: "devbox-01", procRoot: t.TempDir()}
	event, ok := collector.normalize(raw)
	if !ok {
		t.Fatal("expected failed fchmodat event to be normalized")
	}
...
func TestNormalizeChmodRecordResolvesFchmodatPathCrossArch(t *testing.T) {
	procRoot := t.TempDir()
